...
export function AdminServiceManager({ initialArticles }: AdminServiceManagerProps) {
  ...
  return (
    <>
            ...
            <tbody>
              {articles.map((article) => (
                <tr key={article.id}>
                  ...
                  <td><button className="table-button" type="button" aria-label={`编辑${article.title}`}><ArrowUpRight size={15} aria-hidden="true" /></button></td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <AdminDialog open={dialogOpen} title="新建员工服务内容" description="选择上传制度文档，或直接创建可搜索的页面内容。" size="large" onClose={() => setDialogOpen(false)}>
        ...
        <form className="admin-form" action={createContent}>
          <div className="admin-form__grid">
            <label><span>分类</span><select className="form-control" name="category" defaultValue="HR"><option value="HR">HR</option><option value="行政">行政</option><option value="IT">IT</option></select></label>
            <label><span>状态</span><select className="form-control" name="status" defaultValue="draft"><option value="draft">保存草稿</option><option value="published">立即发布</option></select></label>
          </div>
          <label><span>标题</span><input className="form-control" name="title" placeholder="例如：补充医疗保险申请指引" /></label>
          <label><span>摘要</span><textarea className="form-control" name="summary" rows={2} placeholder="一句话说明内容适用范围" /></label>
          <label><span>标签</span><input className="form-control" name="tags" placeholder="使用逗号分隔，例如：保险，报销" /></label>
          {creationMode === 'page' ? <label><span>页面正文</span><textarea className="form-control" name="content" rows={7} placeholder="输入制度说明、办理步骤和注意事项" /></label> : <label className="document-upload-field"><FileText size={22} aria-hidden="true" /><span><strong>选择制度文档</strong><small>文件会在正式环境上传至私有存储；当前原型保存文件名称与内容记录。</small></span><input name="document" type="file" accept=".pdf,.doc,.docx,application/pdf,application/msword,application/vnd.openxmlformats-officedocument.wordprocessingml.document" /></label>}
