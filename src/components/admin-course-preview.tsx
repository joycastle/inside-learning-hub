      ...
      <div className="admin-course-preview__units">
        {course.units.length ? course.units.map((unit) => (
          <article className="admin-course-preview__unit" key={unit.id}>
            ...
            {unit.type === 'video' ? (unit.resources?.length ? unit.resources.map((resource) => resourceType(resource.mimeType, resource.filename) === 'video' ? <div key={resource.id}><video className="admin-course-preview__video" controls playsInline preload="metadata" src={resource.url} /><a className="document-preview__fallback" href={downloadUrl(resource.url)} download>下载视频</a></div> : <DocumentPreview key={resource.id} title={resource.title} type={resourceType(resource.mimeType, resource.filename)} url={resource.url} />) : unit.videoUrl || unit.externalUrl ? <div><video className="admin-course-preview__video" controls playsInline preload="metadata" src={unit.videoUrl ?? unit.externalUrl} /><a className="document-preview__fallback" href={downloadUrl(unit.videoUrl ?? unit.externalUrl ?? '')} download>下载视频</a></div> : <div className="surface empty-state"><p>当前单元尚未关联视频资源。</p></div>) : null}
