...
export function ReferenceDocumentManager({ initialItems }: { initialItems: ReferenceDocumentItem[] }) {
  ...
  const openNew = () => setEditing({ id: '', title: '', summary: '', tags: [], required: false, media: null })
  ...
  return <>
    ...
    <section className="admin-panel admin-panel--flush"><div className="management-list">{visibleItems.length ? visibleItems.map((item) => <div className="management-row management-row--simple" key={item.id}><FileText size={20} aria-hidden="true" /><div className="management-row__body"><strong>{item.title}</strong><p>{item.summary || '暂无说明'} · {fileTypeOf(item.media)}</p></div>{item.required ? <span className="status-badge status-badge--info">必读</span> : null}<div className="management-row__actions"><button className="table-action" type="button" onClick={() => setEditing(item)}><Pencil size={14} aria-hidden="true" />编辑</button><button className="table-action table-action--danger" type="button" onClick={() => void remove(item)}><Trash2 size={14} aria-hidden="true" />删除</button></div></div>) : <div className="empty-state empty-state--compact"><p>暂无参考文档。</p></div>}</div></section>
    <AdminDialog open={Boolean(editing)} title={editing?.id ? '编辑参考文档' : '新增参考文档'} description="新人可以从参考文档入口直接阅读，不依附培训课程。" size="large" onClose={() => !saving && setEditing(null)} footer={<><button className="button button--quiet" type="button" onClick={() => setEditing(null)} disabled={saving}>取消</button><button className="button button--primary" type="submit" form="reference-document-form" disabled={saving}>{saving ? '保存中…' : '保存文档'}</button></>}>{editing ? <form className="admin-form" id="reference-document-form" action={save}><label><span>文档标题</span><input className="form-control" name="title" defaultValue={editing.title} required /></label><label><span>文档说明</span><textarea className="form-control" name="summary" defaultValue={editing.summary} rows={3} required /></label><label><span>标签（可选）</span><input className="form-control" name="tags" defaultValue={(editing.tags ?? []).map(String).join('，')} placeholder="制度，流程，工具" /></label><label><span>替换文件</span><input className="form-control" name="file" type="file" accept=".html,.htm,.pdf,.doc,.docx,.mp4,image/*" /><small>{mediaIdOf(editing.media) ? '不选择文件则保留当前文件；上传新文件会立即切换，旧文件保留 7 天。' : '支持 HTML、PDF、Word、视频和图片。'}</small></label><label className="checkbox-field"><input name="required" type="checkbox" defaultChecked={editing.required} /><span>标记为新人必读</span></label></form> : null}</AdminDialog>
  </>
}
