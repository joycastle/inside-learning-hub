...
export function AdminAnnouncementManager({ initialAnnouncements }: { initialAnnouncements: ManagedAnnouncement[] }) {
  ...
  const save = async (formData: FormData) => {
    ...
    const next = { id: result.id ?? editing?.id, title, audience: audience === 'newEmployees' ? '近 30 天入职员工' : audience === 'departments' ? '指定部门' : '全员', publishedAt: status === '已发布' ? new Date().toISOString() : '—', status }
  ...
  return <>
    ...
    <AdminDialog open={open} title={editing ? '编辑公告' : '新建公告'} description="公告会根据受众和有效期显示在员工首页。" onClose={() => { setOpen(false); setEditing(null) }}>
      <form className="admin-form" action={save}><label><span>标题</span><input className="form-control" name="title" defaultValue={editing?.title ?? ''} placeholder="例如：差旅与费用报销规范已更新" /></label><label><span>摘要</span><textarea className="form-control" name="summary" rows={3} placeholder="填写公告摘要" /></label><label><span>展示人群</span><select className="form-control" name="audience" defaultValue="all"><option value="all">全员</option><option value="newEmployees">近 30 天入职员工</option><option value="departments">指定部门</option></select></label><label><span>目标链接（可选）</span><input className="form-control" name="targetUrl" placeholder="/services/…" /></label><label><span>状态</span><select className="form-control" name="status" defaultValue={editing?.status ?? '草稿'}><option value="草稿">保存草稿</option><option value="已发布">立即发布</option></select></label><div className="admin-dialog__footer admin-dialog__footer--inline"><button className="button button--quiet" type="button" onClick={() => setOpen(false)}>取消</button><button className="button button--primary" type="submit">保存公告</button></div></form>
    </AdminDialog>
  </>
}
