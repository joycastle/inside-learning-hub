...
export function MarkdownDocumentPreview({ title, url, initialContent }: { title: string; url?: string; initialContent?: string }) {
  ...
  if (error) {
    return (
      <div className="document-preview document-preview__fallback">
        <p>{error}</p>
        {url ? <a className="button button--primary" href={url} target="_blank" rel="noreferrer" download>下载 Markdown 文件<ExternalLink size={14} aria-hidden="true" /></a> : null}
      </div>
    )
  }
  ...
  const article = markdown.replace(new RegExp(`^#\\s+${escapedTitle}\\s*\\n+`), '')

  return (
    <div className="document-preview document-preview--markdown">
      <article className="markdown-document-content" aria-label={`${title}内容`}>
        <ReactMarkdown>{article}</ReactMarkdown>
      </article>
      {url ? (
        <a className="document-preview__fallback" href={url} target="_blank" rel="noreferrer" download>
          下载原文<ExternalLink size={14} aria-hidden="true" />
        </a>
      ) : null}
    </div>
  )
