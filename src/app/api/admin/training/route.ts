...
const schema = z.discriminatedUnion('kind', [
  ...
  z.object({ kind: z.literal('course'), pathId: z.string().min(1), title: z.string().trim().min(1), summary: z.string().trim().default(''), category: z.string().trim().default('新员工必看'), unitTitle: z.string().trim().optional(), unitType: z.enum(['video', 'article', 'pdf', 'feishuDoc']).default('video') }),
])
...
export async function POST(request: Request) {
  ...
  if (parsed.data.unitTitle) {
    const unit = await payload.create({ collection: 'units', data: { title: parsed.data.unitTitle, course: createdCourse.id, order: 1, description: parsed.data.summary, type: parsed.data.unitType, durationMinutes: 0 }, overrideAccess: true })
  ...
  return NextResponse.json({ id: createdCourse.id }, { status: 201 })
}
