...
export default async function ReferenceDocumentPage({ params }: { params: Promise<{ documentId: string }> }) {
  ...
  return <div className="page-container handout-page"><Link className="handout-page__back" href="/documents"><ArrowLeft size={16} aria-hidden="true" />返回参考文档</Link><article className="reference-document-page"><header className="onboarding-document__header"><div><span className="eyebrow">参考文档</span><h1>{document.title}</h1><p>{document.summary}</p></div><span className="document-version">更新于 {new Intl.DateTimeFormat('zh-CN', { dateStyle: 'medium' }).format(new Date(document.updatedAt))}</span></header>{document.mediaUrl ? <DocumentPreview title={document.title} type={previewType} url={document.mediaUrl} /> : document.fileType === 'markdown' ? <MarkdownDocumentPreview title={document.title} initialContent={document.body || ''} /> : <div className="reference-document-page__body">{document.html ? <div dangerouslySetInnerHTML={{ __html: document.html }} /> : <p>{document.body || '暂无文档内容。'}</p>}</div>}</article></div>
}
